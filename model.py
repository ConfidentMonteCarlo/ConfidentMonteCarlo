...
class PosEstimator:
    ...
    def __init__(self, sample, realsize=None, probabilistic_model = False, nn = True):
        # realsize is a hack to make plot_restrictions work.
        # don't use unless you know what you're doing!
        if probabilistic_model == True and nn == True:
            self.logprobs = logprobs = np.fromiter((lp for pw, lp in sample), float)
        elif probabilistic_model == True and nn == False:
            self.logprobs = logprobs = np.fromiter((lp for lp, _ in sample), float)
        else:
            self.logprobs = logprobs = sample
        logprobs.sort()
        if realsize is None:
    ...
    def position(self, logprob, inclusive=True):
        if inclusive:
            idx = bisect.bisect_right(self.logprobs, logprob)
        else:
            idx = bisect.bisect_left(self.logprobs, logprob)
